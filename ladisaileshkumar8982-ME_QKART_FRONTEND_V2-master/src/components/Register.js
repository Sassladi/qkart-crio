 ...
 function handleChange(event){
  ...
  const validateInput = (data) => {
    if(!data.username){
      ...
      enqueueSnackbar("username should have atleast 6 characters",{variant:"warning"})
    }
    else if(!data.password){
      enqueueSnackbar("password is required field",{variant:"warning"})
    }
    if(data.password.length<6){
      enqueueSnackbar("password should have atleast 6 characters",{variant:"warning"})
    }
    if(data.password!==data.confirmPassword){
      console.log("password")
      enqueueSnackbar("Passwords do not match",{variant:"warning"})
    }
    if(data.username.length>=6 && data.password===data.confirmPassword ){
       register(formData)
    }
