  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        placeholder="Enter your complete address"
        onChange={(event) => {
          handleNewAddress((currNewAddress) => ({...currNewAddress, value: event.target.value}));
        }}
      />
      <Stack direction="row" my="1rem">
        <Button
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      // TODO: CRIO_TASK_MODULE_CHECKOUT - Add new address to the backend and display the latest list of addresses
      const request=await axios.post(`${config.endpoint}/user/addresses`,{"address":newAddress},{
      ...
      setAddresses({...addresses, all :request.data});
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      const newBalance = parseInt(window.localStorage.getItem("balance"))-getTotalCartValue(items)
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Divider />
            {/* {console.log("zxcvbn")}
            {console.log(addresses.all.length)} */}
            {addresses.all.length>0 && (
                addresses.all.map((item)=>(
                  ...
                  >
                    ...
                    <Button variant="outlined" startIcon={<DeleteIcon />} onClick={()=>{deleteAddress(token,item._id)}}>
                      Delete
                    </Button>
                  </Box>
